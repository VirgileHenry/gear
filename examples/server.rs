...
impl ServerHandler<NetworkMessages> for MyServer {
    fn on_client_connected(&mut self, client: usize, components: &mut ComponentTable) -> Vec<ServerMessage<NetworkMessages>> {
        // for all clients : send new player arrived with given id and position 0
        let mut result = vec![
            ServerMessage::TcpToExcept(client, NetworkMessages::SpawnPlayer(client, false, 0.0)),
            ServerMessage::TcpToClient(client, NetworkMessages::SpawnPlayer(client, true, 0.0)),
        ];
        ...
        for current_player in iterate_over_component!(components; Player) {
            result.push(ServerMessage::TcpToClient(client, NetworkMessages::SpawnPlayer(current_player.id, false, current_player.position)));
        }
    ...
    fn on_client_disconnected(&mut self, client: usize, components: &mut ComponentTable) -> Vec<ServerMessage<NetworkMessages>> {
        // remove player component
        // todo : not yet handled by the foundry
        
        // tell all clients to destroy disconnected player
        vec![
            ServerMessage::TcpToAll(NetworkMessages::DeletePlayer(client))
        ]
    }

    fn update(&mut self, components: &mut ComponentTable, delta: f32) -> Vec<ServerMessage<NetworkMessages>> {
        // nothing to do in this system... no physics to run or whatever
        Vec::new()
    }

    fn handle_tcp_message(&mut self, client: usize, message: NetworkMessages, components: &mut ComponentTable) -> Vec<ServerMessage<NetworkMessages>> {
        ...
        match message {
            NetworkMessages::MovePlayer(player_id, new_pos) => {
                ...
                if client == player_id {
                    ...
                    vec![
                        ServerMessage::TcpToExcept(client, NetworkMessages::MovePlayer(client, new_pos))
                    ]
                }
                else {
                    // don't send anything
