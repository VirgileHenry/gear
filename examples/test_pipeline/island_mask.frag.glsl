...
uniform float time;
uniform float global_falloff;
uniform float falloff_speed;
uniform vec4 chunk;
uniform vec3 islands;
...
void main() {
    ...
    float fact = 0.;
    float falloff = islands.z*global_falloff;
    float noise_mask = perlin(vec3(IN.UV*10.+vec2(.1, .3)*time*100., 0));
    fact = max(fact, noise_mask*(1.-smoothstep(islands.z, islands.z+falloff, length(IN.UV - islands.xy))));

    FragColor = vec4(vec3(fact*falloff_speed*0.5+0.5)/20., 1.);
}